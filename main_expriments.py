...
def train_epoch(model, train_data, optim, opt,obtaini):
    ...
    G_mi_Kn = ConstructHW.constructHW_knn(concat_mi_tensor.detach().cpu().numpy(), K_neigs=[13], is_probH=False)
    G_mi_Km = ConstructHW.constructHW_kmean(concat_mi_tensor.detach().cpu().numpy(), clusters=[7])
    ...
    true_value_one, true_value_zero, pre_value_one, pre_value_zero = test(model, train_data, concat_mi_tensor,
                                                                          concat_dis_tensor,
                                                                          G_mi_Kn, G_mi_Km, G_dis_Kn, G_dis_Km,obtaini)

    return true_value_one, true_value_zero, pre_value_one, pre_value_zero


def test(model, data, concat_mi_tensor, concat_dis_tensor, G_mi_Kn, G_mi_Km, G_dis_Kn, G_dis_Km,obtaini):
    model.eval()
    score, _, _ = model(concat_mi_tensor, concat_dis_tensor,
                        G_mi_Kn, G_mi_Km, G_dis_Kn, G_dis_Km)
    #计算得分矩阵
    # score_np = score.detach().cpu().numpy()  # 转换为 NumPy 数组
    # pd.DataFrame(score_np).to_csv(f'prediction_fold_{obtaini}.csv', index=False, header=False)
...
def main(opt):
    ...
    for i in range(opt.validation):
        ...
        # # 保存真实标签和预测值
        # y_true = np.concatenate([np.ones(len(true_score_one)), np.zeros(len(true_score_zero))])
        # y_scores = np.concatenate([pre_score_one.cpu().detach().numpy(), pre_score_zero.cpu().detach().numpy()])
        #
        # # 计算 ROC 曲线和 AUC
    ...
    # # # 绘制 ROC 曲线
    # # # 绘制 ROC 曲线
    # plot2.plot_roc_curves_with_mean(all_fprs, all_tprs, all_aucs)
