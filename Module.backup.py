...
class HGCLAMIR(nn.Module):
    def __init__(self, mi_num, dis_num, hidd_list, num_proj_hidden, hyperpm):
        ...
        #1
        # 批标准化层
        self.batch_norm_mi = nn.BatchNorm1d(120)
        #1
        self.batch_norm_dis = nn.BatchNorm1d(120)
        #1
        # MMConv 模块
    ...
    def forward(self, concat_mi_tensor, concat_dis_tensor, G_mi_Kn, G_mi_Km, G_dis_Kn, G_dis_Km):
        ...
        # MMConv 处理 miRNA 特征
        mi_mmconv_feature1 = self.mmconv_mi1(mi_concat_feature, G_mi_Kn, mi_concat_feature, lamda=0.011, alpha=0.11, l=1.0)
        mi_mmconv_feature2 = self.mmconv_mi2(mi_embedded, G_mi_Kn, mi_embedded, lamda=0.011, alpha=0.11, l=1.0)
        ...
        dis_mmconv_feature1 = self.mmconv_dis1(dis_concat_feature, G_dis_Kn, dis_concat_feature, lamda=0.011, alpha=0.11, l=1.0)
        dis_mmconv_feature2 = self.mmconv_dis2(dis_embedded, G_dis_Kn, dis_embedded, lamda=0.011, alpha=0.11, l=1.0)
