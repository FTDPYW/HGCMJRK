...
class CL_HGCN(nn.Module):
    def __init__(self, in_size, hid_list, num_proj_hidden, alpha=0.8):
        ...
        self.hgcn1 = HGCN(in_size-2, hid_list)
        self.hgcn2 = HGCN(in_size-2, hid_list)
...
class HGCLAMIR(nn.Module):
    def __init__(self, mi_num, dis_num, hidd_list, num_proj_hidden, hyperpm):
        ...
        #1
        self.mmconv_mi2 = MMConv(in_features=1442, out_features=1442, moment=3, use_center_moment=True)
       #1 auc 94.85  aupr 94.97 pre 87.89
        self.KAN = KAN([128, 64])
        self.linearmi2 = nn.Linear(1442,1024)
        #1

        self.mmconv_dis1 = MMConv(in_features=1024, out_features=1024, moment=3, use_center_moment=True)
        #1
        self.mmconv_dis2 = MMConv(in_features=1442, out_features=1442, moment=3,
                                 use_center_moment=True)
        #1
        self.lineardis2 = nn.Linear(1442, 1024)
        #1 pre 83
        # 全连接层
